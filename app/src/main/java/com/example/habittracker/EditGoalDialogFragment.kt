...
class EditGoalDialogFragment(
    ...
    private val onGoalUpdated: (String, String, String) -> Unit
) : DialogFragment() {
    ...
    private fun populateFields() {
        etGoal.setText(goal.name)

        // Set spinner selections based on current goal
        val periodPosition = when (goal.target) {
            7 -> 0    // 1 Week
            30 -> 1   // 1 Month
            90 -> 2   // 3 Months
        ...
        val habitTypePosition = when (goal.frequency) {
            "Everyday" -> 0
            "Weekly" -> 1
            "Custom" -> 2
